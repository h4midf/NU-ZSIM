...
bool analysis_threads [MAX_THREADS];
...
VOID PIN_FAST_ANALYSIS_CALL IndirectLoadSingle(THREADID tid, ADDRINT addr) {
    if(analysis_running && analysis_threads[tid]){
        // printf("Indirect load single\n");
...
*/
    ...
    if ((INS_Opcode(ins) == XED_ICLASS_CPUID) && (analysis_running)) {
    ...
    // if (INS_IsRDTSC(ins)) {
    if (INS_IsRDTSC(ins) && analysis_running) {
...
VOID Trace(TRACE trace, VOID *v) {
    ...
    if (analysis_running){
        for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
            ...
            // BBL_InsertCall(bbl, IPOINT_BEFORE , (AFUNPTR)IndirectBasicBlock, IARG_FAST_ANALYSIS_CALL,
...
VOID HandleMagicOp(THREADID tid, ADDRINT op) {
    switch (op) {
        case SET_MONITORING_THREAD:
            analysis_threads[tid] = true;
            return;       
        ...
        case ANALYSIS_END:
            ...
            //     if(isMonitoringThread[i]){
            //         int coreID = getCid(tid);    
            //         zinfo->sched->leave(procIdx, tid, coreID);
            //         isMonitoringThread[i] = false; 
            //         SimThreadFini(tid);
            //         fPtrs[tid] = GetFFPtrs();
            //     }
        ...
        case NDP_ROI_END:
            ...
            // }
            // isNDPThread[tid] = false;
            // recallCPUThread(tid);
            return;
        case NDP_FUNC_BEGIN:
            

            // printf("is monitored %d\n", isMonitoringThread[tid]);
            return;
        case NDP_FUNC_END:
